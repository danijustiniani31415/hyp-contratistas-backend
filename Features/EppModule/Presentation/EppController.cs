...
namespace Abril_Backend.Features.EppModule.Presentation
{
    ...
    public class EppController : ControllerBase
    {
        ...
        [HttpGet]
        public async Task<IActionResult> List([FromQuery] int? personaId, [FromQuery] int page = 1, [FromQuery] int pageSize = 20)
        {
            try { return Ok(await _service.List(personaId, page, pageSize)); }
            catch (AbrilException ex) { return StatusCode(ex.StatusCode, new { message = ex.Message }); }
            catch (Exception) { return StatusCode(500, new { message = "Error del servidor. Por favor contactar al administrador del sistema." }); }
        }
